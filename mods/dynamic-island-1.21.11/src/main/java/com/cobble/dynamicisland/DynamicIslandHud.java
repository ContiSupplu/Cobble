...
public class DynamicIslandHud implements HudRenderCallback {
    ...
    // Tab list minimize state — set by PlayerListHudMixin each frame
    public static boolean tabListVisibleThisFrame = false;
    private float tabMinimizeProgress = 0f; // 0 = normal, 1 = minimized to corner
    private float currentX = -1f; // -1 = use centered (default)
    ...
    public void onHudRender(DrawContext ctx, RenderTickCounter tickCounter) {
        ...
        boolean showMusicRecording = false;
        String recTimer = "";
        ...
        if (combatActive && isCombatRelevantAlert) {
        ...
        } else if (hasMusic && CaptureManager.isRecording()) {
            // ── Combined Music + Recording phase ──
            showMusicRecording = true;
            ...
            // Recording timer
            long recMs = System.currentTimeMillis() - CaptureManager.getRecordingStartTime();
            int recSecs = (int)(recMs / 1000);
            recTimer = String.format("%02d:%02d", recSecs / 60, recSecs % 60);

            int textW = Math.max(font.getWidth(line1), font.getWidth(line2));
            int musicSection = 5 + 22 + 6 + textW + 10;
            int recSection = 70; // dot + REC + timer
            targetW = Math.max(240, musicSection + recSection);
            targetH = 34f;
            ...
            // Recording phase — dedicated red pill
            showRecording = true;
            long recMs = System.currentTimeMillis() - CaptureManager.getRecordingStartTime();
            int recSecs = (int)(recMs / 1000);
            int mm = recSecs / 60;
            int ss = recSecs % 60;
            center = String.format("%02d:%02d", mm, ss);
            targetW = 140;
            targetH = 30f;
        ...
        // ── Tab List Minimize Detection ──────────────
        // tabListVisibleThisFrame is set by PlayerListHudMixin when vanilla tab list renders
        boolean tabVisible = tabListVisibleThisFrame && !pebbleOpen && !notifOpen && !settingsOpen && !mediaOpen && !drawerOpen;
        tabListVisibleThisFrame = false; // Reset for next frame

        float tabMinTarget = tabVisible ? 1f : 0f;
        tabMinimizeProgress += (tabMinTarget - tabMinimizeProgress) * 0.18f;
        if (tabMinimizeProgress < 0.005f) tabMinimizeProgress = 0f;
        if (tabMinimizeProgress > 0.995f) tabMinimizeProgress = 1f;

        // When tab list is visible, shrink to a small dot
        if (tabMinimizeProgress > 0.01f) {
            float minW = 20f;
            float minH = 20f;
            targetW = targetW + (minW - targetW) * tabMinimizeProgress;
            targetH = targetH + (minH - targetH) * tabMinimizeProgress;
        }
        ...
        // X position: center normally, top-left corner when minimized
        float centeredX = screenW / 2f - currentWidth / 2f;
        float cornerX = 6f;
        float targetXPos = centeredX + (cornerX - centeredX) * tabMinimizeProgress;
        if (currentX < 0) currentX = centeredX; // init on first frame
        currentX += (targetXPos - currentX) * 0.15f;

        // Y: slide to corner when minimized
        float cornerY = 6f;
        if (tabMinimizeProgress > 0.01f) {
            currentY += (cornerY - currentY) * tabMinimizeProgress * 0.15f;
        }
        ...
        else if (showRecording) bgColor = 0x1A0808;
        else if (showMusicRecording && albumTintColor != 0x0A0A0A) bgColor = albumTintColor;
        ...
        // When minimizing for tab list, clip content to the shrinking pill and fade out
        boolean tabMinimizing = tabMinimizeProgress > 0.01f;
        float contentAlpha = 1f - tabMinimizeProgress;
        if (tabMinimizing) {
            ctx.enableScissor(x, y, x + w, y + h);
            if (tabMinimizeProgress > 0.85f) {
                // Fully minimized — just the dot, skip content
                ctx.disableScissor();
                return;
            }
        }
      ...
      if (pebbleProgress < 0.3f && notifCenterProgress < 0.3f && settingsProgress < 0.3f && mediaProgress < 0.3f && appDrawerProgress < 0.3f) {
        ...
        // Browser background playback indicator
            ...
            // ── Recording phase — premium red-tinted pill ──
            float pulse = 0.5f + 0.5f * (float) Math.sin(System.currentTimeMillis() / 500.0);
            int centerY = y + (h - 7) / 2;

            // ── Red glow aura (renders BEHIND the pill) ──
            int glowAlpha = (int)(20 + 28 * pulse);
            int glowColor = (glowAlpha << 24) | 0xFF4444;
            drawRoundedRect(ctx, x - 2, y - 2, w + 4, h + 4, radius + 2, glowColor);

            // ── Re-draw pill background over glow ──
            drawRoundedRect(ctx, x, y, w, h, radius, bg);

            // ── Subtle red inner border ──
            int borderAlpha = (int)(40 + 30 * pulse);
            drawRoundedRectOutline(ctx, x, y, w, h, radius, (borderAlpha << 24) | 0xFF5555);

            // ── Pulsing red dot (circular via drawRoundedRect with r=3) ──
            int dotSize = 6;
            int dotX = x + 8;
            int dotY = centerY - 1;
            int dotAlpha = (int)(180 + 75 * pulse);
            int dotColor = (dotAlpha << 24) | 0xFF3B3B;
            drawRoundedRect(ctx, dotX, dotY, dotSize, dotSize + 2, 3, dotColor);

            // Bright center of the dot
            int coreAlpha = (int)(220 + 35 * pulse);
            drawRoundedRect(ctx, dotX + 1, dotY + 1, dotSize - 2, dotSize, 2, (coreAlpha << 24) | 0xFF5555);

            // ── "REC" label with subtle glow ──
            int recColor = (0xFF << 24) | 0xFF5555;
            String recLabel = "\u00a7lREC";
            ctx.drawTextWithShadow(font, recLabel, dotX + dotSize + 5, centerY, recColor);

            // ── Timer (right-aligned, white) ──
            int timerW = font.getWidth(center);
            int timerX = x + w - timerW - 10;
            ctx.drawTextWithShadow(font, center, timerX, centerY, 0xFFEEEEEE);

            // ── Thin separator line between REC and timer ──
            int recEndX = dotX + dotSize + 5 + font.getWidth(recLabel);
            int sepX = recEndX + (timerX - recEndX) / 2;
            ctx.fill(sepX, y + 6, sepX + 1, y + h - 6, 0x30FFFFFF);
        } else if (showMusicRecording) {
            // ═══════════════════════════════════════════════════════════════
            // Combined Music + Recording — premium dual-section pill
            // LEFT 1/3:  pulsing red dot + REC + elapsed timer
            // RIGHT 2/3: album art + title + artist + EQ + progress
            // ═══════════════════════════════════════════════════════════════
            long now = System.currentTimeMillis();
            float pulse = 0.5f + 0.5f * (float) Math.sin(now / 500.0);
            float slowPulse = 0.5f + 0.5f * (float) Math.sin(now / 1200.0);

            // ── Divider position (35% | 65%) — recording | music ──
            int dividerX = x + (int)(w * 0.35f);

            // ── Outer glow aura (pulsing red, wraps entire pill) ──
            int auraAlpha = (int)(8 + 14 * slowPulse);
            drawRoundedRect(ctx, x - 2, y - 2, w + 4, h + 4, radius + 2, (auraAlpha << 24) | 0xFF4040);

            // ── Re-draw main pill bg cleanly over aura ──
            drawRoundedRect(ctx, x, y, w, h, radius, bg);

            // ── Red tint wash on the LEFT recording section ──
            int redTintAlpha = (int)(18 + 14 * slowPulse);
            // Fill left section with red tint (manual clip to pill bounds)
            for (int row = 2; row < h - 2; row++) {
                float edgeFade = 1f;
                if (row < 5) edgeFade = row / 5f;
                else if (row > h - 5) edgeFade = (h - row) / 5f;
                int rowAlpha = (int)(redTintAlpha * edgeFade);
                ctx.fill(x + 2, y + row, dividerX - 1, y + row + 1, (rowAlpha << 24) | 0xFF2020);
            }

            // ── Pulsing red border (subtle, full pill) ──
            int borderAlpha = (int)(20 + 18 * pulse);
            drawRoundedRectOutline(ctx, x, y, w, h, radius, (borderAlpha << 24) | 0xFF4444);

            // ═══ LEFT SECTION: Recording ═══════════════════════════
            int recCenterY = y + (h - 7) / 2;
            int leftPadding = x + 8;

            // ── Outer glow ring behind the dot ──
            int glowRingAlpha = (int)(30 + 40 * pulse);
            int glowRingSize = 12;
            int glowRingX = leftPadding - 3;
            int glowRingY = recCenterY - 4;
            drawRoundedRect(ctx, glowRingX, glowRingY, glowRingSize, glowRingSize, 6,
                    (glowRingAlpha << 24) | 0xFF3030);

            // ── Main red dot (circular, pulsing brightness) ──
            int dotSize = 6;
            int dotAlpha = (int)(200 + 55 * pulse);
            drawRoundedRect(ctx, leftPadding, recCenterY - 1, dotSize, dotSize + 2, 3,
                    (dotAlpha << 24) | 0xFF3B3B);
            // Hot white-ish inner core
            int coreAlpha = (int)(180 + 75 * pulse);
            drawRoundedRect(ctx, leftPadding + 1, recCenterY, dotSize - 2, dotSize, 2,
                    (coreAlpha << 24) | 0xFF6060);

            // ── "REC" label — bold, with breathing opacity ──
            int recLabelX = leftPadding + dotSize + 5;
            int recAlpha = (int)(200 + 55 * pulse);
            ctx.drawTextWithShadow(font, "\u00a7lREC", recLabelX, recCenterY,
                    (recAlpha << 24) | 0xFF4444);

            // ── Timer — white, right-aligned within left section ──
            int timerW2 = font.getWidth(recTimer);
            int timerX2 = dividerX - timerW2 - 8;
            ctx.drawTextWithShadow(font, recTimer, timerX2, recCenterY, 0xFFCCCCCC);

            // ═══ DIVIDER: Animated gradient line (red → white shimmer) ═══
            int divLineH = h - 10;
            for (int dy = 0; dy < divLineH; dy++) {
                float t = dy / (float) divLineH;
                // Red at top, fading to white-ish at bottom
                int dr = (int)(255 * (0.9f - 0.3f * t));
                int dg = (int)(255 * (0.2f + 0.4f * t));
                int db = (int)(255 * (0.2f + 0.4f * t));
                float shimmer = 0.3f + 0.7f * (float) Math.sin((now / 600.0) + t * 4.0);
                int da = (int)(50 * shimmer);
                ctx.fill(dividerX, y + 5 + dy, dividerX + 1, y + 6 + dy,
                        (da << 24) | (dr << 16) | (dg << 8) | db);
            }

            // ═══ RIGHT SECTION: Music ═════════════════════════════
            int musicStartX = dividerX + 6;
            int artSize = 22;
            int artX = musicStartX;
            int artY = y + (h - artSize) / 2;

            // Album art (rounded)
            if (albumTextureId != null && !albumLoading) {
                ctx.drawTexture(RenderPipelines.GUI_TEXTURED, albumTextureId,
                        artX, artY, 0, 0, artSize, artSize, artSize, artSize);
            ...
            // Equalizer bars (right-aligned within music section)
            int eqX = x + w - 20;
            int eqY = y + 8;
            if (musicPlaying) {
            ...
            // Progress bar (spans music section only)
            int barY = y + h - 4;
            int barX = musicStartX + 2;
            int barW = x + w - barX - 6;
            ctx.fill(barX, barY, barX + barW, barY + 2, 0x25FFFFFF);
            int filled = (int)(barW * displayProgress);
            if (filled > 0) ctx.fill(barX, barY, barX + filled, barY + 2, 0x99FFFFFF);
        ...
        // (Tab player list: DI minimizes to corner, vanilla tab list renders normally)

        // End tab-minimize scissor clip before panels
        if (tabMinimizing) {
            ctx.disableScissor();
        }
        ...
        if (mediaOpen && mediaProgress > 0.01f) {
            ...
            if (InGameBrowser.isActive) {
                ...
                if (InGameBrowser.activeMode == InGameBrowser.Mode.MEDIA) {
                    int tabY = y + 3;
                    boolean twitchAvailable = InGameBrowser.isTwitchSupported();
                    ...
                    // Twitch tab — only show if H.264 codecs are available (Classic MCEF)
                    int twX = ytX + ytW + 3;
                    int twW = 0;
                    if (twitchAvailable) {
                        int twLabelW = font.getWidth("Twitch");
                        int twLogoW = 12;
                        twW = twLogoW + twLabelW + 8;
                    ...
                    if (hasClick) {
                        ...
                        if (clickY >= tabY && clickY < tabY + 12) {
                            ...
                            else if (twitchAvailable && clickX >= twX && clickX < twX + twW) InGameBrowser.switchTab("twitch");
                        }
                ...
                boolean browserRendered = false;

                if (InGameBrowser.isUsingModernApi()) {
                    // MCEF Modern: register GpuTexture with TextureManager and draw
                    if (InGameBrowser.updateBrowserTexture() && bw > 0 && bh > 0) {
                        browserFrameCount++;
                        ctx.drawTexture(RenderPipelines.GUI_TEXTURED, BROWSER_TEX_ID, bx, by, 0f, 0f, bw, bh, bw, bh);
                        browserRendered = true;
                        InGameBrowser.resizeBrowser();
                    }
                } else {
                    // Classic MCEF: render via texture ID
                ...
                if (!browserRendered) {
                    if (InGameBrowser.statusMessage != null) {
                        int msgY = by + bh / 2 - 10;
            ...
            if (isSearchTab) {
                ...
                if (MediaViewer.isSearching) {
                    ...
                    for (int ri = 0; ri < pageResults.size(); ri++) {
                        ...
                        if (thumbId != null) {
                            ctx.drawTexture(RenderPipelines.GUI_TEXTURED, thumbId, thumbX, thumbY2, 0, 0, thumbW, thumbH, thumbW, thumbH);
                        } else {
                ...
                if (MediaViewer.hasTexture() && MediaViewer.getTextureIdentifier() != null) {
                    ctx.drawTexture(RenderPipelines.GUI_TEXTURED, MediaViewer.getTextureIdentifier(), videoX, videoTop, 0, 0, videoW, videoH, videoW, videoH);
                } else {
                ...
                if (resp.statusCode() == 200) {
                    ...
                    MinecraftClient.getInstance().execute(() -> {
                        try {
                            ...
                            // 1.21.11: NativeImageBackedTexture(id, width, height, blur)
                            NativeImageBackedTexture texture = new NativeImageBackedTexture("album_art", image.getWidth(), image.getHeight(), false);
