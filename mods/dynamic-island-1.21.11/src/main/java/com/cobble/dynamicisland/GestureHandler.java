...
public class GestureHandler {
    ...
    public static void register() {
        ScreenEvents.AFTER_INIT.register((client, screen, scaledWidth, scaledHeight) -> {
            ...
            ScreenMouseEvents.beforeMouseRelease(screen).register((s, click) -> {
                if (click.button() == 0 && tracking) {
                    tracking = false;
                    double mx = click.x();
                    double my = click.y();
                    double dx = mx - startX;
    ...
    private static boolean isProtectedItem(ItemStack stack) {
        ...
        // Check for sword-like items and armor by checking if item is damageable
        // and has equippable component or is a known weapon
    ...
    private static int getCategoryOrder(ItemStack stack) {
        Item item = stack.getItem();
        // Swords are no longer a separate class in 1.21.11, check by name
        String name = item.toString().toLowerCase();
        if (name.contains("sword")) return 0;
        if (item instanceof AxeItem || item instanceof HoeItem || item instanceof ShovelItem) return 1;
        if (item instanceof BowItem || item instanceof CrossbowItem || item instanceof TridentItem) return 2;
        // Armor check: items with equippable component in armor slots
        if (stack.isDamageable() && !stack.isEmpty() && item instanceof ShieldItem) return 5;
        if (item == Items.ELYTRA) return 4;
        if (item == Items.TOTEM_OF_UNDYING) return 5;
        ...
        if (stack.isDamageable()) return 3;  // Other damageable items (armor, tools)
        return 8;
    }
}
