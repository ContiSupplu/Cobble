    ...
    public void onMessage(String message) {
        try {
            ...
            if ("state".equals(state.type)) {
                ...
                // Quick and dirty way to parse Pebble answer without a dedicated class
                java.util.Map map = GSON.fromJson(message, java.util.Map.class);
                String ans = (String) map.get("text");
                if (ans != null) {
                    net.minecraft.client.MinecraftClient.getInstance().execute(() -> {
                        if (net.minecraft.client.MinecraftClient.getInstance().currentScreen instanceof PebbleScreen) {
                            ((PebbleScreen) net.minecraft.client.MinecraftClient.getInstance().currentScreen).setAnswer(ans);
                        }
                    });
                }
            }
